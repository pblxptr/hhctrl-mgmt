...
template<class R, class Func, class... Args, class CompletitionHandler = boost::asio::use_awaitable_t<>>
auto awaitable_call2(Func&& func, Args&&... args, CompletitionHandler&& handler = {})
{
  using Signature_t = void(std::exception_ptr, R);

  auto initiate = [func = std::forward<Func>(func)]<class Handler>(Handler&& self, auto&&... args) mutable {
    std::invoke(func, std::forward<decltype(args)>(args)..., [self = std::make_shared<Handler>(std::forward<Handler>(self))](auto&& r, auto&&... args) {
      (*self)(std::current_exception(), std::forward<decltype(r)>(r), std::forward<decltype(args)>(args)...);
    });
  };
  return boost::asio::async_initiate<CompletitionHandler, Signature_t>(initiate, handler);
