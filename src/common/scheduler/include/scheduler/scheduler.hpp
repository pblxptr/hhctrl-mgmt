...
namespace {
  ...
  template<class T>
  requires std::is_same_v<std::decay_t<T>, days_at>
  inline std::string duration_to_str(T&& arg)
  {
  ...
  typename std::decay_t<TDuration>::period >> || std::is_same_v<std::decay_t<TDuration>, days_at> inline std::stringstream& operator<<(std::stringstream& ss, TDuration&& duration)
  {
...
public:
  ...
  // EverydayAt task
  template<class TOwner, class TDuration, class THandler>
  requires std::is_same_v<std::decay_t<TDuration>, days_at>
  auto every(TOwner&& owner, Execution policy, TDuration&& duration, THandler&& handler)
  {
    auto task_id = generate_id(owner, duration);
